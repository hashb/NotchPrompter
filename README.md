...
Requires Xcode 14+ and Node.js.

```bash
./build.sh
```

To notarize and staple — run this once to store credentials in Keychain, then set `NOTARIZE=true`:
...
The DMG is written to `build/NotchPrompter-<version>.dmg`.
